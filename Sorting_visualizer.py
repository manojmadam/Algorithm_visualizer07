...
def generate():
    ...
    for bar in range(1, 60):
        ...
        bar = canvas.create_rectangle(barstart, randomY, barend, 365, fill='#3b3bff')
        barList.append(bar)
        barstart += 10
        barend += 10
...
window = tk.Tk()
window.title('Sorting Visualizer - Manoj Madam')
...
#Buttons
insert = tk.Button(window, text='Insertion Sort', command=insertion_sort, width=18, height=2)
select = tk.Button(window, text='Selection Sort', command=selection_sort, width=19, height=2)
bubble = tk.Button(window, text='Bubble Sort', command=bubble_sort, width=20, height=2)
shuf = tk.Button(window, text='Shuffle', command=generate, bg = "#9d37e6", fg = "white", width=21, height=2)
